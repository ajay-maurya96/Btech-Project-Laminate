function validateLoanLiabilityLedger(input) {
  ...
  if (input.parent !== "Loans (Liability)") {
    throw new Error("parent must be 'Loans (Liability)'");
  }
  ...
  if (!Array.isArray(input.languageNames) || input.languageNames.length === 0) {
    throw new Error("languageNames are required");
  }
  ...
  // Banking is mandatory (because PAYMENTDETAILS.LIST exists)
  if (!input.paymentDetails) {
    throw new Error("paymentDetails are mandatory for Loans (Liability)");
  }
...
module.exports = validateLoanLiabilityLedger;
